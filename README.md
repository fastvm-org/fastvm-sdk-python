...
to add `FASTVM_API_KEY="My API Key"` to your `.env` file
...
When the library is unable to connect to the API (for example, due to network connection problems or a timeout), a subclass of `fastvm.APIConnectionError` is raised.

When the API returns a non-success status code (that is, 4xx or 5xx
response), a subclass of `fastvm.APIStatusError` is raised, containing `status_code` and `response` properties.

All errors inherit from `fastvm.APIError`.
...
except fastvm.RateLimitError as e:
    print("A 429 status code was received; we should back off a bit.")
except fastvm.APIStatusError as e:
...
You can enable logging by setting the environment variable `FASTVM_LOG` to `info`.

```shell
$ export FASTVM_LOG=info
```
...
```python
import httpx
from fastvm import Fastvm, DefaultHttpxClient

client = Fastvm(
    # Or use the `FASTVM_BASE_URL` env var
